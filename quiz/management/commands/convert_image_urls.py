...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if revert:
            self.stdout.write(
                self.style.WARNING("REVERTING: Adding CloudFront domain to paths")
            )
        else:
            self.stdout.write(
                self.style.WARNING("CONVERTING: Removing CloudFront domain from URLs")
            )
        ...
        for question in questions:
            ...
            if question.question_image_url:
                if not revert and question.question_image_url.startswith(
                    cloudfront_domain
                ):
                    new_url = question.question_image_url.replace(cloudfront_domain, "")
                    self.stdout.write(
                        f"Question {question.id} image: {question.question_image_url} -> {new_url}"
                    )
                    if not dry_run:
                        question.question_image_url = new_url
                        changed = True
                elif revert and not question.question_image_url.startswith(
                    cloudfront_domain
                ):
            ...
            if question.answer_image_url:
                if not revert and question.answer_image_url.startswith(
                    cloudfront_domain
                ):
                    new_url = question.answer_image_url.replace(cloudfront_domain, "")
                    self.stdout.write(
                        f"Question {question.id} answer image: {question.answer_image_url} -> {new_url}"
                    )
                    if not dry_run:
                        question.answer_image_url = new_url
                        changed = True
                elif revert and not question.answer_image_url.startswith(
                    cloudfront_domain
                ):
        ...
        for answer in answers:
            ...
            if answer.question_image_url:
                if not revert and answer.question_image_url.startswith(
                    cloudfront_domain
                ):
                    new_url = answer.question_image_url.replace(cloudfront_domain, "")
                    self.stdout.write(
                        f"Answer {answer.id} image: {answer.question_image_url} -> {new_url}"
                    )
                    if not dry_run:
                        answer.question_image_url = new_url
                        changed = True
                elif revert and not answer.question_image_url.startswith(
                    cloudfront_domain
                ):
            ...
            if answer.answer_image_url:
                if not revert and answer.answer_image_url.startswith(cloudfront_domain):
                    ...
                    self.stdout.write(
                        f"Answer {answer.id} answer image: {answer.answer_image_url} -> {new_url}"
                    )
                    if not dry_run:
                        answer.answer_image_url = new_url
                        changed = True
                elif revert and not answer.answer_image_url.startswith(
                    cloudfront_domain
                ):
        ...
        if dry_run:
            self.stdout.write(
                self.style.SUCCESS(
                    f"DRY RUN: Would update {question_count} questions and {answer_count} answers"
                )
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully updated {question_count} questions and {answer_count} answers"
                )
            )
