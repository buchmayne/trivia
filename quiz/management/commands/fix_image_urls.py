    ...
    def handle(self, *args, **options):
        ...
        for answer in Answer.objects.all():
            ...
            if answer.answer_image_url and re.search(pattern, answer.answer_image_url):
                ...
                self.stdout.write(
                    f"Answer {answer.id} answer_image_url:\n  {answer.answer_image_url} → \n  {new_url}"
                )
                if not dry_run:
            ...
            if answer.question_image_url and re.search(
                pattern, answer.question_image_url
            ):
                new_url = re.sub(pattern, replacement, answer.question_image_url)
                self.stdout.write(
                    f"Answer {answer.id} question_image_url:\n  {answer.question_image_url} → \n  {new_url}"
                )
                if not dry_run:
        ...
        for question in Question.objects.all():
            ...
            # Check question_image_url
            if question.question_image_url and re.search(
                pattern, question.question_image_url
            ):
                new_url = re.sub(pattern, replacement, question.question_image_url)
                self.stdout.write(
                    f"Question {question.id} question_image_url:\n  {question.question_image_url} → \n  {new_url}"
                )
                if not dry_run:
            ...
            # Check answer_image_url
            if question.answer_image_url and re.search(
                pattern, question.answer_image_url
            ):
                new_url = re.sub(pattern, replacement, question.answer_image_url)
                self.stdout.write(
                    f"Question {question.id} answer_image_url:\n  {question.answer_image_url} → \n  {new_url}"
                )
                if not dry_run:
