...
def get_game_questions(request, game_id):
    ...
    return Response(
        {"game": GameDetailSerializer(game).data, "questions": serializer.data}
    )
...
def add_team_to_session(request, session_id):
    ...
    team = SessionTeam.objects.create(session=session, team_name=team_name)

    return Response(
        {"team_id": team.id, "team_name": team.team_name, "joined_at": team.joined_at}
    )
...
def finalize_session(request, session_id):
    ...
    for team_data in teams_data:
        SessionTeam.objects.filter(session=session, team_name=team_data["name"]).update(
