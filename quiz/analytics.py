...
def get_players_list(spreadsheet_url: str, trivia_metadata: dict) -> list:
    ...
    players_list_sheet_names = [
        trivia_metadata[game]["player_list_sheet_name"] for game in trivia_metadata
    ]
    return (
        pd.concat(
            [
                read_google_sheet(spreadsheet_url=spreadsheet_url, sheet_name=sheet)
                for sheet in players_list_sheet_names
            ],
            axis=0,
        )
        .sort_values(by="name")[["name", "gender"]]
        .drop_duplicates()
...
def process_game_results(
    spreadsheet_url: str, game_sheet_name: str, game_data: dict
) -> pd.DataFrame:
    ...
    game_result = (
        ...
        .assign(
            ...
            zscore_total=lambda df_: (df_["Total"] - df_["Total"].mean())
            / df_["Total"].std(),
...
def get_player_stats(game_results: pd.DataFrame, players: list) -> pd.DataFrame:
    ...
    return pd.concat(
        [
            (
                game_results.pipe(exact_player_match, player)
                .reset_index(drop=True)
...
def calculate_player_performance(players_stats: pd.DataFrame) -> pd.DataFrame:
    ...
    return (
        ...
        .sort_values(
            ["avg_final_place", "total_wins", "avg_zscore_total_points"],
...
if __name__ == "__main__":
    ...
    game_results_df = get_game_results(
        spreadsheet_url, trivia_metadata
    )  # included in analytics view
    ...
    career_stats_df = calculate_player_performance(
        players_stats_df
    )  # included in analytics view
